#!/usr/bin/env python3
"""
Generate text-change edit instructions at scale using vLLM + Qwen VL.
...
Usage:
    python generate_text_change_instructions.py \
        --image-paths balanced_sampled_image_paths_200000.txt \
        --model Qwen/Qwen3-VL-30B-A3B-Instruct \
        --output-dir text_change_instructions \
        --world-size 1 \
        --rank 0 \
        --tensor-parallel 8
"""
...
DEFAULT_OUTPUT_DIR = "text_change_instructions"
...
# ---------------------------------------------------------------------------
# Categories and target sampling ratios (from gedit_bench_text_change_guide.md)
# ---------------------------------------------------------------------------
CATEGORIES: list[tuple[str, float]] = [
    ("Single Text Replacement/Rewriting", 0.70),
    ("Text Addition", 0.14),
    ("Text Deletion", 0.12),
    ("Multi-text Replacement/Composite Rewrite", 0.02),
    ("Delete Then Rewrite", 0.02),
]
...
# ---------------------------------------------------------------------------
# Prompts adapted from gedit_bench_text_change_guide.md
# ---------------------------------------------------------------------------
SYSTEM_PROMPT = """\
You are a high-precision instruction generator for text editing inside images.

Your task is to inspect the input image and generate one high-quality image-edit instruction for a PRE-SPECIFIED text_change category. The target category is provided externally by the data pipeline. You must not switch categories.

Your output will be used to train an image editing model. Therefore, the instruction must be realistic, visually grounded, benchmark-aligned, and directly executable as a user-style text editing request.

You must strictly follow all rules below.

GENERAL GOAL
- Given one image and one externally assigned target category, generate one natural instruction that belongs only to that category.
- The instruction should resemble a realistic user request for editing text inside an existing image.
- The instruction should improve category coverage for benchmark-style text_change tasks.
...
- Single Text Replacement/Rewriting
- Text Addition
- Text Deletion
- Multi-text Replacement/Composite Rewrite
- Delete Then Rewrite

CATEGORY DEFINITIONS
- Single Text Replacement/Rewriting:
  change one visible text target into different text. This includes explicit source-to-target replacement and context-anchored single-target rewrites such as changing text on a shirt, sign, tattoo, poster, package, screen, or board.
- Text Addition:
  add new text to the image without deleting existing text.
- Text Deletion:
  remove existing text without adding new replacement text.
- Multi-text Replacement/Composite Rewrite:
  edit two or more distinct text targets in one instruction.
- Delete Then Rewrite:
  explicitly remove old text first and then write new text into that cleared region.

VISUAL GROUNDING RULES
- Ground every instruction in what is actually visible in the image.
- Only target visible text, clearly localizable text regions, or plausible nearby blank space when the category is Text Addition.
- Do not invent unreadable source text unless the region is clearly identifiable by context and the category naturally allows context-anchored phrasing.
- Do not request edits on regions that are severely occluded or too small or blurry to support reliable text editing.
- If multiple text regions exist, be precise about which one is being edited.

PRESERVATION RULES
- Keep non-text image content unchanged unless the text edit necessarily affects a tiny local area.
- Preserve layout, background, object identity, camera viewpoint, lighting, and scene content.
- Prefer localized text edits over broad scene rewrites.
- Do not turn the request into a full image generation prompt.

STYLE RULES
- Output natural, concise, user-style edit requests.
- Use imperative phrasing.
- Each instruction should be 5 to 24 words.
- Avoid prompt-engineering style language.
- Avoid vague wording such as "change the words" without enough target detail.
- Avoid mentioning category labels inside the instruction.
- Prefer quoted text when the source or target text is explicit.
- Prefer one instruction only. If multiple edits are required, they must still belong to the assigned category.

APPLICABILITY RULES
- Single Text Replacement/Rewriting: applicable when one editable text target is visible or clearly localizable.
- Text Addition: applicable when there is a plausible place to add text while preserving the rest of the image.
- Text Deletion: applicable when visible text exists to remove.
- Multi-text Replacement/Composite Rewrite: applicable only when two or more distinct editable text targets are present.
- Delete Then Rewrite: applicable when old text is visible and the instruction can naturally require clearing it before writing new text.

SAFETY RULES
- Do not generate hateful, slur-based, violent, sexual, self-harm, extremist, or otherwise unsafe target text.
- Do not generate defamatory or harassing text.
- Do not introduce copyrighted slogans or branded content excessively unless it is a short, realistic, benchmark-style example already common in ordinary user requests.

DIVERSITY RULES
- Generate natural, non-generic instructions.
- In Single Text Replacement/Rewriting, vary between words, numbers, short phrases, and context-anchored rewrites.
- In Text Addition, vary position constraints and occasional style constraints.
- In Text Deletion, mix deleting explicit text and deleting all text from a local region.
- Keep the instruction benchmark-like rather than overly creative.
...
- Only edit visible or clearly localizable text regions.
- If the requested category is not visually applicable, return applicable=false.
- Keep non-text image content unchanged.
- Prefer natural benchmark-style text editing requests.
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Generate text-change edit instructions with vLLM + Qwen VL.",
    ...
    p.add_argument(
        "--all-results-output",
        type=str,
        default="text_change_all_results.parquet",
        help="Output parquet with ALL outcomes (for audit / rebalancing).",
    )
    p.add_argument(
        "--checkpoint-dir",
        type=str,
        default="text_change_instruction_checkpoints",
...
def validate_result(parsed: dict, assigned_category: str) -> dict[str, Any]:
    ...
    if category != assigned_category or category not in _VALID_CATEGORIES:
        return {"status": "invalid", "instruction": None}
    ...
    if applicable is False:
        return {"status": "not_applicable", "instruction": None}
...
def merge_checkpoints(checkpoint_dir: Path) -> pd.DataFrame:
    ...
    return pd.DataFrame(
        columns=["image_path", "category", "status", "instruction"]
    )
...
def main() -> int:
    ...
    ).reindex(columns=["image_path", "category", "instruction"])
    ...
    applicable_df = applicable_df[
        ["row_idx", "image_path", "category", "instruction", "width", "height"]
    ]
